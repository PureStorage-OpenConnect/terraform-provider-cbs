...
func (a *Client) GetAPI24Sessions(params *GetAPI24SessionsParams) (*GetApi24SessionsOK, error) {
	...
	if params == nil {
		params = NewGetAPI24SessionsParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "GetAPI24Sessions",
		Method:             "GET",
		PathPattern:        "/api/2.4/sessions",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &GetAPI24SessionsReader{formats: a.formats},
	...
	success, ok := result.(*GetApi24SessionsOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for GetAPI24Sessions: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
