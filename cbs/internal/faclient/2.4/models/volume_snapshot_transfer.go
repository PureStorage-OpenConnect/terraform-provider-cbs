// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// VolumeSnapshotTransfer volume snapshot transfer
//
// swagger:model VolumeSnapshotTransfer
type VolumeSnapshotTransfer struct {
	Resource

	Transfer

	VolumeSnapshotTransferAllOf2
}

// UnmarshalJSON unmarshals this object from a JSON structure
func (m *VolumeSnapshotTransfer) UnmarshalJSON(raw []byte) error {
	// AO0
	var aO0 Resource
	if err := swag.ReadJSON(raw, &aO0); err != nil {
		return err
	}
	m.Resource = aO0

	// AO1
	var aO1 Transfer
	if err := swag.ReadJSON(raw, &aO1); err != nil {
		return err
	}
	m.Transfer = aO1

	// AO2
	var aO2 VolumeSnapshotTransferAllOf2
	if err := swag.ReadJSON(raw, &aO2); err != nil {
		return err
	}
	m.VolumeSnapshotTransferAllOf2 = aO2

	return nil
}

// MarshalJSON marshals this object to a JSON structure
func (m VolumeSnapshotTransfer) MarshalJSON() ([]byte, error) {
	_parts := make([][]byte, 0, 3)

	aO0, err := swag.WriteJSON(m.Resource)
	if err != nil {
		return nil, err
	}
	_parts = append(_parts, aO0)

	aO1, err := swag.WriteJSON(m.Transfer)
	if err != nil {
		return nil, err
	}
	_parts = append(_parts, aO1)

	aO2, err := swag.WriteJSON(m.VolumeSnapshotTransferAllOf2)
	if err != nil {
		return nil, err
	}
	_parts = append(_parts, aO2)
	return swag.ConcatJSON(_parts...), nil
}

// Validate validates this volume snapshot transfer
func (m *VolumeSnapshotTransfer) Validate(formats strfmt.Registry) error {
	var res []error

	// validation for a type composition with Resource
	if err := m.Resource.Validate(formats); err != nil {
		res = append(res, err)
	}
	// validation for a type composition with Transfer
	if err := m.Transfer.Validate(formats); err != nil {
		res = append(res, err)
	}
	// validation for a type composition with VolumeSnapshotTransferAllOf2

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

// MarshalBinary interface implementation
func (m *VolumeSnapshotTransfer) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *VolumeSnapshotTransfer) UnmarshalBinary(b []byte) error {
	var res VolumeSnapshotTransfer
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}

// VolumeSnapshotTransferAllOf2 The transfer statistics for snapshot replication data.
//
// swagger:model VolumeSnapshotTransferAllOf2
type VolumeSnapshotTransferAllOf2 interface{}
